...
RSpec.describe Product, type: :model do
  describe 'Validations' do
    ...
    it 'should validate that a name is not nil' do
      @category = Category.new(name: 'Category')
      @product = Product.new(name: nil, quantity: 5, price: 599, category: @category)
      expect(@product).to_not be_valid
    end

    it 'should validate that a price is not nil' do
      @category = Category.new(name: 'Category')
      @product = Product.new(name: 'Name', quantity: 5, price: '', category: @category)
      expect(@product).to_not be_valid
    end

    it 'should validate that a quantity is not nil' do
      @category = Category.new(name: 'Category')
      @product = Product.new(name: 'Name', quantity: nil, price: 599, category: @category)
      expect(@product).to_not be_valid
    end

    it 'should validate that a category is not nil' do
      @product = Product.new(name: 'Name', quantity: 5, price: 599, category: nil)
      expect(@product).to_not be_valid          
    end

    it 'should validate that an error message is generated when an attribute is set to nil' do
